...
export(NodePath) var sky_view_path
export(NodePath) var env_path
export(NodePath) var sun_path
export(NodePath) var clouds_view_path
export(NodePath) var sky_tex_path
export(NodePath) var clouds_tex_path
...
func _ready():
	...
	env = get_node(env_path).environment
	sun = get_node(sun_path)
	sky_view = get_node(sky_view_path)
	clouds_view = get_node(clouds_view_path)
	sky_tex = get_node(sky_tex_path)
	clouds_tex = get_node(clouds_tex_path)
...
func reflections_update():
	...
	#var imgData = sky_view.get_texture().get_data()

	#var image = Image.new()
	#var imageTexture = ImageTexture.new()

	#image.copy_from(imgData)
	#imageTexture.create_from_image(image)
	#env.background_sky.set_panorama(imageTexture)
